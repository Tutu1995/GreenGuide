		  ...
		  try{
				  ...
				  //if(is_uploaded_file($_FILES['image']['tmp_name'])){
						if(isset($_FILES['image']['tmp_name'])){	
				  				...
				  				if($img_size>8388608){
						  			redirect("WriteReview.php", "Sorry, image size is bigger than 8MB. Please rewrite the review again.");
				  				}else{

				  					  $result=$con->prepare("insert into review (company, address, city, review, lng, lat, rating, water, air, waste, land, living, other, news, industry, product, epa, measure) values(:company, :address, :city, :review, :lng, :lat, :rating, :water, :air, :waste, :land, :living, :other, :news, :industry, :product, :epa, :measure)");					    
						  		  ...
						  		  //echo "image OK! ";		
									  foreach($_FILES['image']['tmp_name'] as $key => $tmp_name){
										  //echo "tmp_name is: " . $tmp_name;
									  			if($_FILES['image']['tmp_name'][$key]){
                                                          ...
                                                          $result=$con->prepare("insert into image (image,review_id,size) values(:image, :last_id, :size)");
                                                          $result->bindParam(':image', $image);
                                                          $result->bindParam(':last_id', $last_id);
									  ...
									  $size=$con->prepare("UPDATE review SET size=:size where id=:id");
					...
					//}
								  ...
								  $result=$con->query("insert into profile (name,review_id) values ('$s_name','$last_id')");

								  $suppliers=$con->query("select * from com_sup where sup_name='$company' and sup_lng='$lng' and sup_lat='$lat'");
								  if($suppliers){
								  		foreach($suppliers as $supplier){
								  				$insert_com_review=$con->query("insert into com_review (com_id, review_id) values ($supplier[1], '$last_id')");
								  				sup_mail($company, $address, $city, $review, $lng, $lat, $rating, $water, $air, $waste, $land, $living, $other, $news);
								  		}
								  }
								  $_SESSION["review_id"] = $last_id;
								  header("Location: http://greenguide.azurewebsites.net/confirm_review.php");
