<?php  
		  ...
		  $status_token = md5(uniqid(rand(),TRUE));
		  $_SESSION["status_token"] = $status_token; 
		  ...
		  try{
				  ...
				  $rows=$db->query("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(r.id) AS num_r from review r join profile p on r.id= p.review_id where r.status IS NULL GROUP BY lng,lat,company ");
				  ...
				  $reviews=$db->query("select * from review where status IS NULL order by id DESC ");
				  $all=array();
