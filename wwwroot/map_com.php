<?php  
		  ...
		  try{
				  ...
				  $rows = $con->prepare("select * from review where company=:company and lng=:lng and lat=:lat and status=1 order by id DESC LIMIT 10 OFFSET 0 ");
				  ...
				  $r_count = $con->prepare("select COUNT(id) from review where company=:company and lng=:lng and lat=:lat and status=1 ");
				  ...
				  $p_map=$con->query("select company, address, city, lng, lat, AVG(rating) as avg_r, COUNT(id) as num_r from review where company='$company' and lng='$lng' and lat='$lat' and status=1 GROUP BY lng,lat,company ");
				  $a=array();
				  foreach ($p_map as $p) {
						$a[]=$p;
						$r_company=$p[0];
