      ...
      <div class="wrapper">
				...
				-->
                  <div id="mycity"> 
		                 ...
		                 -->
		                  <div id="r-result"><strong>Please search the city or company in the dropdown menu and click it. System'll bring the city or company location on the map: </strong><input type="text" id="suggestId_2" size="37" placeholder="Company Name or (Location + Company Name)"  /></div>
                  ...
                  <div id="allmap"></div>
                  <h2 style="margin-top: 10px;"><span style="color:grey; font-style: italic; font-family: Times New Roman; font-size: 32px; ">Location History - </span>Click a mark on the above map and check out its reviews:</h2>
...
<?php
			...
			try{
				  ...
				  //$rows=$con->query("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where status=1 GROUP BY lng,lat,company ");
				  

				  	if ($industry && !($product)) {
	
						$rows=$con->prepare("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where industry like ? and status=1 GROUP BY lng,lat,company");					    
						$rows->execute(array('%'.$industry.'%'));
									
					}
					if (!($industry) && $product) {
						
						$rows=$con->prepare("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where product like ? and status=1 GROUP BY lng,lat,company");					    
						$rows->execute(array('%'.$product.'%'));
							
					}
					if ($industry && $product) {
						
						$rows=$con->prepare("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where industry like ? and product like ? and status=1 GROUP BY lng,lat,company");					    
						$rows->execute(array('%'.$industry.'%', '%'.$product.'%'));
