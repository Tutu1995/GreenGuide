<?php  
		  ...
		  $report_token = md5(uniqid(rand(),TRUE));
		  $_SESSION["report_token"] = $report_token; 
		  ...
		  try{
				  ...
				  $rows=$con->query("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(r.id) AS num_r from review r join profile p on r.id= p.review_id where r.report IS NOT NULL GROUP BY lng,lat,company ");
				  ...
				  $reviews=$con->query("select * from review where report IS NOT NULL order by id DESC ");
				  $all=array();
...
<script type="text/javascript">
	...
	var report_token = <?php echo json_encode($report_token) ?>;

	document.getElementById("profile").className="r_visited";

	var count=document.getElementById("count");
	count.innerHTML=all.length+" Reviews"
	...
	*/
	...
	function setList(page){
			...
			for (var i = 0; i < all_page.length; i++) {
				...
				info.className="info";
				info.setAttribute("id", "info"+all_page[i].review.id );
				...
				*/				
		    			...
		    			btn.onclick=btnClick_public;
		    			btn.setAttribute("id", "btn_public"+all_page[i].review.id );

		    			


		    			var btn_i = document.createElement("BUTTON");				
		    			btn_i.innerHTML="Postponed";   			
		    			btn_i.className="btn_i";
		    			btn_i.data = {				
						  id: all_page[i].review.id,
						  r_status: 2				  	  
						}
		    			btn_i.onclick=btnClick_public;
		    			btn_i.setAttribute("id", "btn_post"+all_page[i].review.id );
		    			...
		    			btn_d.setAttribute("id", "btn_d"+all_page[i].review.id );
			...
			function view() {
					...
					function btnClick_public(){
							toggleLoadingMessage();
							var id=this.data.id;
							var r_status=this.data.r_status;

							//alert("btnClick_public: status"+r_status);					
							
					        var params = new FormData();
							params.append("id", id);
							params.append("r_status", r_status);			
							
							var ajax = new XMLHttpRequest();
							ajax.onload = status;
							ajax.open("POST", "status.php", true);
							ajax.send(params);
					}
					...
					function status() {
						...
						if (this.status == 200) {
							...
							//alert(this.responseText);
							var json = JSON.parse(this.responseText);
							//alert("json response json_id"+json.id);
							//var show=document.getElementById(json.id);
							//show.innerHTML="This review is deleted!";	
							//var info_d=document.getElementById("info"+json.id );        
		          			//list.removeChild(info_d);
		          			document.getElementById("btn_public"+json.id).disabled = true;
		          			document.getElementById("btn_public"+json.id).style.color = "gray";

		          			document.getElementById("btn_post"+json.id).disabled = true;
		          			document.getElementById("btn_post"+json.id).style.color = "gray";
