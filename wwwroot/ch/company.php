<?php  
		  ...
		  try{
				  ...
				  $rows = $db->prepare("select * from review where company=:company and lng=:lng and lat=:lat and status=:status order by id DESC LIMIT 10 OFFSET 0 ");
				  //$rows=$con->query("select * from review where company='$company' and lng='$lng' and lat='$lat' order by id DESC ");
				  $rows->bindParam(':lng', $lng);
				  $rows->bindParam(':lat', $lat);
				  $rows->bindParam(':company', $company);
				  $rows->bindParam(':status', $s_status);
				  $rows->execute();

				  //$r_count=$con->query("select COUNT(id) from review where lng='$lng' and lat='$lat' and status=1");

				  $r_count = $db->prepare("select COUNT(id) from review where company=:company and lng=:lng and lat=:lat and status=:status ");
				  //$r_count = $con->prepare("select COUNT(id) from review where company=:company and lng=:lng and lat=:lat");
				  $r_count->bindParam(':lng', $lng);
				  $r_count->bindParam(':lat', $lat);
				  $r_count->bindParam(':company', $company);
				  $r_count->bindParam(':status', $s_status);
				  $r_count->execute();
				  ...
				  $p_map=$db->query("select company, address, city, lng, lat, AVG(rating) as avg_r, COUNT(id) as num_r from review where company='$company' and lng='$lng' and lat='$lat' and status='$s_status' GROUP BY lng,lat,company ");
				  //$p_map=$con->query("select company, address, city, lng, lat, AVG(rating) as avg_r, COUNT(id) as num_r from review where company='$company' and lng='$lng' and lat='$lat' GROUP BY lng,lat,company ");
				  $a=array();
				  foreach ($p_map as $p) {
						$a[]=$p;
						$r_company=$p[0];
...
<script type="text/javascript">
	...
	if(data.length>0){
				...
				*/
				var count_c=document.getElementById("count_c");
				count_c.innerHTML=r_count+" 点评 - " + all[0].review.company + "  - 评分: "+ parseFloat(rate).toFixed(2);
				//count.className="count";
				count_c.className="text-warning";
				count_c.style.borderBottom="1px solid silver";
				count_c.style.marginTop = "3px";
...
function setList(page){
			...
			for (var i = 0; i < all_page.length; i++) {
				...
				var r_info= "公司名称: " + all_page[i].review.company +"<br/>" + "地址: "+ all_page[i].review.address +"<br/>" + "城市: "+all_page[i].review.city+"<br/>" + "产业类别: " +all_page[i].review.industry+ "<br/>" + "产品名称: " +all_page[i].review.product+ "<br/>"+"评分: "+all_page[i].review.rating+"<br/>" + "<img src='"+ pic[pic_rate] +"' width='228' height='30' /><br/>"+ "公司环境影响评价: "+all_page[i].review.review +"<br/>"+ "环境关注点: "+all_page[i].review.water+" " +all_page[i].review.air+" "+all_page[i].review.waste+" "+all_page[i].review.land+" "+all_page[i].review.living+" "+all_page[i].review.other+"<br/>"+"相关新闻、视频或链接: "+all_page[i].review.news+"<br/>"+ "环保局官方信息: " +all_page[i].review.epa+ "<br/>"+ "实验数据: " +all_page[i].review.measure+ "<br/>"+"提交时间: "+all_page[i].review.time+"<br/>";
				...
				img_c_div.innerHTML="图片数: "+ all_page[i].img_count;
				...
				var btn = document.createElement("BUTTON");
				if(all_page[i].review.help){
    				btn.innerHTML="有帮助 : "+ all_page[i].review.help;
    			}
    			else{
    				btn.innerHTML="有帮助";
    			}
    			...
    			var report = document.createElement("BUTTON");
    			var r = document.createTextNode("举报");
...
function setList_ajax(json){
			...
			for (var i = 0; i < json.all.length; i++) {
				...
				var r_info= "公司名称: " + json.all[i].review.company +"<br/>" + "地址: "+ json.all[i].review.address +"<br/>" + "城市: "+json.all[i].review.city+"<br/>" + "产业类别: "+json.all[i].review.industry+"<br/>" + "产品名称: "+json.all[i].review.product+"<br/>" + "评分: "+json.all[i].review.rating+"<br/>" + "<img src='"+ pic[pic_rate] +"' width='228' height='30' /><br/>"+ "公司环境影响评价: "+json.all[i].review.review +"<br/>"+ "环境关注点: "+json.all[i].review.water+" " +json.all[i].review.air+" "+json.all[i].review.waste+" "+json.all[i].review.land+" "+json.all[i].review.living+" "+json.all[i].review.other+"<br/>"+"相关新闻、视频或链接: "+json.all[i].review.news+"<br/>"+"环保局官方信息: "+json.all[i].review.epa+"<br/>"+"实验数据: "+json.all[i].review.measure+"<br/>"+"提交时间: "+json.all[i].review.time+"<br/>";
				//var r_info= "Company Name: " + all_page[i].review.company +"<br/>" + "Company Address: "+ all_page[i].review.address +"<br/>" + "City: "+all_page[i].review.city+"<br/>" + "Company Industry:" +all_page[i].review.industry+ "<br/>" + "Company Product:" +all_page[i].review.product+ "<br/>"+"Rating: "+all_page[i].review.rating+"<br/>" + "<img src='"+ pic[pic_rate] +"' width='228' height='30' /><br/>"+ "Reviews: "+all_page[i].review.review +"<br/>"+ "Environment Type: "+all_page[i].review.water+" " +all_page[i].review.air+" "+all_page[i].review.waste+" "+all_page[i].review.land+" "+all_page[i].review.living+" "+all_page[i].review.other+"<br/>"+"Related News, Videos, or links: "+all_page[i].review.news+"<br/>"+ "EPA Data:" +all_page[i].review.epa+ "<br/>"+ "Measurement Data:" +all_page[i].review.measure+ "<br/>"+"Time: "+all_page[i].review.time+"<br/>";
				...
				//alert(json.all[i].img_count);
				var img_c_div = document.createElement("p");
				img_c_div.innerHTML="图片数: "+ json.all[i].img_count;
				//img_c_div.className="img_c";
				...
				var btn = document.createElement("BUTTON");
				if(json.all[i].review.help){
    				btn.innerHTML="有帮助 : "+ json.all[i].review.help;
    			}
    			else{
    				btn.innerHTML="有帮助";
    			}
			...
			function rClick(){
				...
				var reason = prompt("请输入该举报的原因，只接受附原因的举报", "");
...
// 百度地图API功能
 ...
 // 定义一个控件类,即function
	...
	ZoomControl.prototype.initialize = function(map){
	  ...
	  // 添加文字说明
	  div.appendChild(document.createTextNode("放大地图"));
	  // 设置样式
	  ...
	  div.onclick = function(e){
		...
		if (map_height.classList.contains('map_s')) {
			...
			div.appendChild(document.createTextNode("缩小地图"));
