<?php  
          ...
          $water=htmlspecialchars(pType("水", $_POST["water"]));
          $air=htmlspecialchars(pType("空气", $_POST["air"]));
          $waste=htmlspecialchars(pType("废弃物", $_POST["waste"]));
          $land=htmlspecialchars(pType("土壤", $_POST["land"]));
          $living=htmlspecialchars(pType("生态系统",$_POST["living"]));
          $other=htmlspecialchars(pType("其他",$_POST["other"]));
		  ...
		  if(!isset($token) || !isset($_SESSION["token"]) || $token !== $_SESSION["token"]) {
				...
				redirect("WriteReview.php", "错误发生, 请重新再试, 或检查上传图片档案大小");
				die();
		  } else {
		  ...
		  try{
				  ...
				  //if(is_uploaded_file($_FILES['image']['tmp_name'])){
						if(isset($_FILES['image']['tmp_name'])){	
				  				...
				  				if($img_size>1048576){
						  			redirect("WriteReview.php", "很抱歉，图片大小超过1 MB。请重新上传图片。");
				  				}else{
					...
					//}
								  ...
								  header("Location: http://www.lovegreenguide.com/ch/confirm_review.php");
