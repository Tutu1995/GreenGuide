...
-->
                  ...
                  <div class="container-fluid"> 
						  ...
						  <form action="map_go.php" class="form-inline" role="form">
							  <div class="form-group">
							    <label for="industry">输入产业类别或产品名称，点击“提交”进行地图搜索 </label>
							    <input id="industry" name="industry" type="text" class="form-control input-sm" size="30" placeholder="产业类别">
							  </div>
							  <div class="form-group">
							  	<label class="sr-only" for="product">product</label>
							    <input id="product" name="product" type="text" class="form-control input-sm" size="30" placeholder="产品名称">
							  </div>
			...
			try{
				  ...
				  //$rows=$con->query("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where status=1 GROUP BY lng,lat,company ");
				  	...
				  	if ($industry && !($product)) {
						...
						$rows=$db->prepare("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where industry like ? and status=1 GROUP BY lng,lat,company");					    
						$rows->execute(array('%'.$industry.'%'));
									
					}
					if (!($industry) && $product) {
						
						$rows=$db->prepare("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where product like ? and status=1 GROUP BY lng,lat,company");					    
						$rows->execute(array('%'.$product.'%'));
							
					}
					if ($industry && $product) {
						
						$rows=$db->prepare("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where industry like ? and product like ? and status=1 GROUP BY lng,lat,company");					    
