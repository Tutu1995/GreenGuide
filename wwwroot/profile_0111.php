<?php  
		 ...
		 // $_SESSION["del_token"] = $del_token; 
		  ...
		  try{
				  ...
				  $rows=$con->query("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(r.id) AS num_r from review r join profile p on r.id= p.review_id where p.name='$s_name' GROUP BY lng,lat,company ");
				  ...
				  $reviews=$con->query("select * from review r join profile p on r.id= p.review_id where p.name='$s_name' order by r.id DESC ");
				  $all=array();
