...
<head>
	...
	<title>Apple Supplier Map</title>
...
<body>
      <div class="wrapper">
                        ...
                        <div id="search">
                                ...
                                <nav>
                                     <ul id="topmenu">
                                          ...
                                          <li id="apple"><a href="Apple.php">Apple Supplier Map</a></li>
                                     </ul> 
                                </nav>
                  ...
                  <div id="mycity"> 
		                 ...
		                 -->
		                  <div id="r-result"><strong>Please enter key words and search on the map. ex. company name or location: </strong><input type="text" id="suggestId_2" size="64" placeholder="Company Name or (Location + Company Name)"  /></div><br/>
                  		  <div id="searchResultPanel_2" style="border:1px solid #C0C0C0;width:150px;height:auto; display:none;"></div>
		          </div>    
                  <div id="allmap"></div>
                  <h2 id="show_review"><span style="color:grey; font-style: italic; font-family: Times New Roman; font-size: 32px; ">Apple Supplier Map - </span>click a mark on the above map and check out the supplier's reviews:</h2>
                  <div id="count"></div>
...
<?php
			try{
				  ...
				  $apple=$con->query("select * from com_sup where com_id='1' ");
				  $a=array();
				  foreach ($apple as $sup){
				  		 $rows=$con->query("select lng, lat, company, address, city, AVG(rating) as avg_r, COUNT(id) as num_r from review where company='$sup[2]' and lng='$sup[3]' and lat='$sup[4]' GROUP BY company,lng,lat ");
				  		 if($rows){	
							  foreach($rows as $row)	
							  {
				  		 			$a[]=$row;
...
<script type="text/javascript">
	...
	document.getElementById("apple").className="a_visited";
	...
	function addClickHandler(content,marker,company){
	...
	function pointInfo(company,e){
		...
		//var params = new FormData();
			...
			ajax.open("GET", "ajax.php?company="+encodeURI(company)+"&lng="+ pi.getPosition().lng +"&lat="+ pi.getPosition().lat+ "&page="+"1", true);
			//ajax.send(params);
			ajax.send();
			//alert("2");
	}
	...
	function pInfoGet() {
		...
		if (this.status == 200) {
			...
			// set page list
			setPage(json.review_count,json.company, json.lng, json.lat, json.page);
			setList(json);

			var count=document.getElementById("count");
			count.innerHTML=json.review_count+" Reviews - "+json.company;
	...
	function setList(json){
			...
			function view() {
						...
						if (this.status == 200) {
								...
								document.getElementById("btn_v"+json.id).style.color="gray";
						}
						else{
							alert("error in ajax");
						}
